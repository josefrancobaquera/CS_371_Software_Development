...
public void run() {
   ...
   try {
      ...
      // We have to check what type of file was inputted in the 
      // browser by the user. We accomplish this by using a set of
      // if-else statements. Print a meaningful message
      // for debugging purposes. 
      if ( pathToContent.toLowerCase().contains(".gif") ) {
         typeOfContent = "image/gif";
         System.err.println("Type of Content Entered: " + typeOfContent );
      } // end if. 
            
      else if ( pathToContent.toLowerCase().contains(".jpeg") ) {
         typeOfContent = "image/jpeg";
         System.err.println("Type of Content Entered: " + typeOfContent );
      } // end else-if
            
      else if ( pathToContent.toLowerCase().contains(".png") ) {
         typeOfContent = "image/png";
         System.err.println("Type of Content Entered: " + typeOfContent );
      } // end else-if
      
      // EXTRA CREDIT: Check for the .ico file.
      else if ( pathToContent.toLowerCase().contains("ico") ) {
      ...
      // Call two methods to "write out" the HTTP header
...
private void writeHTTPHeader(OutputStream os, String contentType, String pathToFile ) throws Exception {
   ...
   try {
      ...
      os.write("Connection: close\n".getBytes());
      os.write("Content-Type: ".getBytes());
      // Make the default type to text/html in order to property print out a text error. 
...
private void writeContent(OutputStream os, String contentType, String pathToFile ) throws Exception {
   ...
   // We need to check if an image was sent (i.e. gif, jpeg, or png). We accomplish this
   ...
   if ( contentType.equals("text/html") ) {
      ...
      try {
         ...
         os.write ( "<b>The file you tried to open does not exists! :(</b><br/>\n".getBytes( ) );
   ...
   // If an image was found, we need to open the file in "byte mode."
   // First check if the type of file sent was type GIF, PNG, etc.    
      ...
      try {
         ...
         os.write ( "<body>\n".getBytes( ) );
         os.write ( "<h1>404 Not Found</h1>\n".getBytes() );
         os.write ( "<b>The IMAGE file you tried to open does not exists! :(</b><br/>\n".getBytes( ) );
         os.write ( "<b>Please make sure that the correct path name was given and/or that the file exists! :)</b>\n".getBytes( ) ); 
         os.write ( "</body>\n".getBytes() );  
         os.write ( "</html>\n".getBytes( ) );               
